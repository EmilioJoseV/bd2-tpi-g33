...
BEGIN
    ...
    BEGIN
        RAISERROR('El id de producto es invalido', 16, 1);
        RETURN;
    END

    IF @IdTipoMovimientoStock IS NULL OR @IdTipoMovimientoStock <= 0
    BEGIN
        RAISERROR('El tipo de movimiento es invalido', 16, 1);
        RETURN;
    END

    IF @IdEmpleado IS NULL OR @IdEmpleado <= 0
    BEGIN
        RAISERROR('El id de empleado es invalido', 16, 1);
        RETURN;
    END
    ...
    BEGIN
        RAISERROR('El producto no esta activo', 16, 1);
        RETURN;
    END
    ...
    BEGIN
        RAISERROR('Ese tipo de movimiento es gestionado automaticamente por el sistema. Use un tipo de ajuste manual.', 16, 1);
        RETURN;
    END
